...
public:
	...
	enum class ConfigureDialogResult { KeepCandidates, RefreshCandidates };

	static void ProcessFiles(const std::vector<std::wstring>& filepaths);
	static ConfigureDialogResult ShowConfigureDialog();
	static void ShowError(const std::vector<std::wstring>& error_lines);
	static void LoadGeneralSettings();

private:

	enum class DetailsDialogResult { Close, Launch };
	...
	static DetailsDialogResult ShowDetailsDialog(const std::vector<std::wstring>& filepaths, const std::vector<std::wstring>& unique_mime_profiles, const std::vector<Field> &application_info, const std::vector<std::wstring>& cmds);

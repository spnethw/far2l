...
namespace openwith
{
	...
	private:
		...
		{
			// Progress text updated by the worker thread; read by the dialog proc.
			std::wstring title;
			std::wstring status;
			std::mutex   mtx_text;

			// Last text values sent to the dialog; used by ProgressDlgProc to avoid
			// redundant DM_SETTEXTPTR calls and to allow clearing a field (empty string).
			// Accessed only from the dialog proc (main thread), no synchronization needed.
			std::wstring last_shown_title;
			std::wstring last_shown_status;

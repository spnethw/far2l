...
namespace openwith
{
	...
	{
		ProgressState state;
		AppProvider::GetCandidatesResult fetch_result;
		...
		std::thread worker([&]() {
			try {
				fetch_result = provider.GetAppCandidates(filepaths, progress_cb, &state.cancelled);
			} catch (...) {
				exception = std::current_exception();
			}

			// Signal completion; wakes the 300 ms wait below.
			{
				std::lock_guard<std::mutex> lock(state.mtx_finish);
				state.finished.store(true);
			}
			state.cv_finish.notify_one();
		...
		{
			// Wait up to 300 ms for the worker to finish.
			std::unique_lock<std::mutex> lock(state.mtx_finish);
			state.cv_finish.wait_for(lock, std::chrono::milliseconds(300), [&]{ return state.finished.load(); });
		}
	...
	LONG_PTR WINAPI Plugin::ProgressDlgProc(HANDLE progress_dlg, int msg, int param1, LONG_PTR param2)
	{
		...
		switch (msg) {
			...
			case DN_ENTERIDLE: {
				if (state->finished.load()) {
					if (!state->close_sent.exchange(true)) {
						g_info.SendDlgMessage(progress_dlg, DM_CLOSE, 0, 0);
					}
				} else {
					std::wstring title, status;
					{
						std::lock_guard<std::mutex> lock(state->mtx_text);
						title  = state->title;
						status = state->status;
